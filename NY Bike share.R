...
#Cleaning the data  
data$Trip.Duration=NULL #Remvoing redundant duration row
...
ggplot(data = data, aes(x=(Start.Hours), fill=factor(User.Type)))+
  geom_bar()+
  xlab("Time of the day (24hr format)")+
  ggtitle("Demand during the day and User type")+
  ylab("Total Count")+
  labs(fill="Customer")
...
data<-data[!(data$User.Type == "Customer"), ]
...
finalruledf = rbind(rule3df, rule5df) # Final set of 38 freuqent patterns
